...
# Magic Rampage — generated by nxbootstrap 0.6.26.
...
nxbootstrap_write_early_log() {
  ...
  for log_dir in "${GAMEDIR:-}" "$NXBOOTSTRAP_LAUNCHER_DIR" \
    ...
    if (umask 077; set -C; printf '%s\n' \
      '== Magic Rampage | nxbootstrap 0.6.26 | pre-runtime failure ==' \
...
if ! echo "== Magic Rampage | nxbootstrap 0.6.26 | cfw=${CFW_NAME:-none} =="; then
  exit 1
fi
...
nxbootstrap_phase_event() {
  ...
  if ! (umask 077; set -C; printf '%s\n' \
    "{\"schema\":\"org.nextos.nxbootstrap.phase-result\",\"schema_version\":1,\"nxbootstrap_version\":\"0.6.26\",\"sequence\":$NXBOOTSTRAP_PHASE_SEQUENCE,\"run_id\":\"magicrampage-$$\",\"source\":\"$source\",\"phase\":\"$phase\",\"boundary\":\"$boundary\",\"status\":\"$event_status\",\"reason_code\":$reason_code,\"child_status\":$child_json,\"pid\":$$}" \
...
if NX_SPLASH_TTY="${CUR_TTY:-}" \
   ...
   $NXBOOTSTRAP_SPLASH_PREFIX "$NXBOOTSTRAP_SPLASH" 'Magic Rampage'; then
